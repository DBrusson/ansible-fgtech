...
### Le prompt avec les conditions
```ansible-playbook -i inventory conditions.yml ```

### les boucles
```ansible-playbook -i inventory loops.yml```
  Is ansible loop too slow ?
...
```ansible-playbook -i inventory new_filter.yml --limit 2.home ```

#### Exemple de filtre: obtenir la derniere version de glusterfs
```shell script
    ansible-playbook -i inventory git_version_filter.yml 
```
...
```ansible-playbook -i inventory ansible_create_module.yml```
